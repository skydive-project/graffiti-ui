...
export class Topology extends React.Component<Props, {}> {
    ...
    pinNode(node: Node, active: boolean) {
        if (active) {
            this.showNode(node)
        }
    ...
    private renderLevels() {
        ...
        levelEnter.transition()
            .duration(animDuration)
            .style("opacity", 1)
            .on('end', (d: LevelRect) => this.showLevelLabel(d))
    ...
    private renderGroups() {
        ...
        groupEnter.each(function (d: NodeWrapper) { handleCurlyBraces(select(this), d, false) })
        group.each(function (d: NodeWrapper) { handleCurlyBraces(select(this), d, true) })
