...
export class App extends React.Component<Props, State> {
  ...
  state: State
  refreshTopology: any
  bumpRevision: Function
  checkAuthID: number
  staticDataURL: string
  wsContext: WSContext
  extraConfigURL: string
  connected: boolean
  debSetState: Function
  ...
  constructor(props: Props) {
    ...
    this.refreshTopology = debounce(300, false, this._refreshTopology.bind(this))

    // we will refresh info each 1s
    this.bumpRevision = debounce(1000, false, this.props.bumpRevision.bind(this))

    // debounce version of setState
    this.debSetState = debounce(200, false, this.setState.bind(this))

    // will handle multiple configuration files
    this.config = new ConfigReducer(this)
    if (this.props.config) {
      this.config.append('default', this.props.config)
    }
  }
  ...
  addEdge(edge: any): boolean {
    ...
    if (parent && child) {
      if (this.config.isParentLink(parent, child, edge.Metadata)) {
        this.tc.setParent(child, parent)
      } else {
        let tags = this.config.linkTags(parent, child, edge.Metadata)
        this.tc.addLink(edge.ID, parent, child, tags, edge.Metadata)
      }
    }
  ...
  onLinkTagStateChange(event: any) {
    if (!this.tc) {
      return
    }
      ...
      <React.Fragment>
        {
          ...
          <Websocket ref={(node: any) => this.websocket = node} url={this.subscriberURL()} onOpen={this.onWebSocketOpen.bind(this)}
          ...
          <Container className={classes.rightPanel}>
            <Paper className={clsx(classes.rightPanelPaper, (!this.props.selection.length || !this.state.isSelectionOpen) && classes.rightPanelPaperClose)}
              ...
              <SelectionPanel onLocation={this.onSelectionLocation.bind(this)} onClose={this.onSelectionClose.bind(this)} config={this.config}
                buttonsContent={this.props.actionButtons} panelsContent={this.props.dataPanels} />
            </Paper>
          </Container>
